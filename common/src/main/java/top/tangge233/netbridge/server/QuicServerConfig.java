...
/**
 * 服务端配置装载与参数解析（{@code net-bridge/server.toml}）。
 *
...
final class QuicServerConfig {
    ...
    /**
     * 读取 {@code net-bridge/server.toml}；文件不存在时从 jar 内置模板
    ...
    static ServerConfig load() {
        ...
        try {
            ...
            try (CommentedFileConfig config = CommentedFileConfig.builder(file)
                    .defaultResource("/net-bridge/server-default.toml")
                    .build()) {
                config.load();
    ...
    /**
     * 解析 QUIC 监听端口：优先级为系统属性 {@code netbridge.quicPort} >
