 ...
 * {@code enable}/{@code bind}/{@code host}/{@code port}/{@code max_connection} （kcp 另有
 * {@code profile}）。文件不存在时从 jar 内置模板 {@code server-default.toml} 自动生成；解析失败告警并按内置默认处理， 不阻断启动。无旧配置迁移。
 */
public final class ServerConfig {
    ...
    public static ServerConfig load() {
        var dir = ClientConfig.configDir();
        if (dir == null) {
            return empty();
        }

        var file = dir.resolve("server.toml");
        try {
            ...
            try (var config = CommentedFileConfig.builder(file).build()) {
                ...
                return new ServerConfig(
                        readSection(
                                config,
                                "quic",
                                SectionDefaults.QUIC.enable(),
                                null
                        ),
                        readSection(
                                config,
                                "kcp",
                                SectionDefaults.KCP.enable(),
                                SectionDefaults.KCP.profile()
                        )
                );
            }
        } catch (Exception e) {
    ...
    /**
     * 首次启动时把 jar 内置模板写到配置文件位置（注释即文档，可直接编辑）； 已存在则不动。模板经本类的 classloader 读取——mod 资源只在 mod 层可见。
     */
    private static void ensureTemplateExists(Path file) {
        ...
        try (var in = ServerConfig.class.getResourceAsStream(TEMPLATE_RESOURCE)) {
            ...
            NetBridge.LOGGER.warn(
                    "Failed to write default server config {}: {}",
                    file,
                    e.toString()
            );
        }
     ...
     *
     * @param defaultEnable 段缺 {@code enable} 字段时的默认值（quic=true，kcp=false——与模板文档一致，缺整段不等于意外开启监听）
     */
    private static Section readSection(
        ...
        var enable = (Boolean) config.get(name + ".enable");
        var bind = (String) config.get(name + ".bind");
        var host = (String) config.get(name + ".host");
        var port = (Integer) config.get(name + ".port");
        var maxConnection = (Integer) config.get(name + ".max_connection");
        var profile = (String) config.get(name + ".profile");
        return new Section(
                enable == null
                        ? defaultEnable
                        : enable,
    ...
    public record Section(
            boolean enable,
            @Nullable String bind,
            @Nullable String host,
            @Nullable Integer port,
            @Nullable Integer maxConnection,
    ...
    private static final class SectionDefaults {
        ...
        static final Section QUIC = new Section(
                true,
                null,
                null,
                -1,
        ...
        static final Section KCP = new Section(
                false,
                null,
                null,
                -1,
