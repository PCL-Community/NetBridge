 ...
 * 否则读平台注册的 TOML 配置文件，默认 TCP。运行时切换（游戏内按钮）
 * 自动写回。文件形态：
 *
 * <pre>
 * mode = "quic"
...
public final class ClientConfig {
    ...
    private static TransportMode initialMode() {
        TransportMode parsed = TransportMode.parse(System.getProperty(PROP_MODE));
        return parsed == null ? TransportMode.TCP : parsed;
    }

    /**
     * 注册配置文件路径并加载其中保存的模式与 profile（平台 mod 入口启动时
     * 调用一次；传 {@code null} 注销注册）。文件缺失/损坏时保持当前值，
     * 不视为错误。
     */
    public static synchronized void useConfigFile(Path file) {
        ...
        if (System.getProperty(PROP_MODE) != null) {
            NetBridge.LOGGER.info(
                    "Transport mode from system property {}, ignoring config file {}", mode(), file);
            return;
        }
        try {
            ...
            TransportMode loadedMode = TransportMode.parse(toml.getString("mode"));
            if (loadedMode != null) {
                mode = loadedMode; // 直接赋值，避免加载过程触发回写
                NetBridge.LOGGER.info("Transport mode loaded from {}: {}", file, loadedMode);
            }
            String profileValue = toml.getString("kcp.profile");
            KcpProfile loadedProfile = KcpProfile.parse(profileValue);
            if (loadedProfile != null) {
                kcpProfile = loadedProfile;
            } else if (profileValue != null) {
                NetBridge.LOGGER.warn(
                        "Unknown kcp profile '{}' in {}: keeping {}", profileValue, file, kcpProfile);
            }
        } catch (Exception e) {
    ...
    public static void setMode(TransportMode newMode) {
        mode = newMode == null ? TransportMode.TCP : newMode;
    ...
    public static void setKcpProfile(KcpProfile profile) {
        kcpProfile = profile == null ? KcpProfile.BALANCE : profile;
        save();
    }
