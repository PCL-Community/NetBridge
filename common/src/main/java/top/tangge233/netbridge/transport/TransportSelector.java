...
public final class TransportSelector {
     ...
     */
    public static Optional<TransportTarget> decide(InetSocketAddress tcpAddress) {
        ...
        NetworksEntry entry = networks.entry(name);
        if (entry == null || !entry.usable()) {
            String reason = entry == null
                    ? (networks.entries().isEmpty() ? "server did not advertise" : "not advertised")
                    : "unsupported protocol or disabled";
            NetBridge.LOGGER.info("Transport for {}: TCP (mode={}, {})", tcpAddress, mode, reason);
            return Optional.empty();
        }
        String host = entry.host() != null ? entry.host() : tcpAddress.getHostString();
        InetSocketAddress endpoint =
                InetSocketAddress.createUnresolved(host, entry.port());
