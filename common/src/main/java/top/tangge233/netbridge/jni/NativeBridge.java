 ...
 * Java 通过同步批量接口读写字节。Rust 侧错误即时输出到 stderr （由启动器重定向进 logs/latest.log）。
 *
...
public final class NativeBridge {
    ...
    /**
     * 注册 Rust→Java 数据到达回调（native 侧保存调用目标）。幂等； 未注册时数据到达仅靠 Java 侧轮询兜底。
     */
    public static native void registerNotifyCallback();

    /**
     * Rust 数据到达通知（tokio 线程回调，仅作唤醒信号，不带数据）。 实际读取由 channel 在自身 EventLoop 上执行，避免跨线程触碰 pipeline。
     */
    public static void onDataAvailable(long connId) {
        var channel = NativeChannel.channelFor(connId);
        if (channel != null) {
            channel.wakeupRead();
        }
    }
     ...
     * @param kcpProfile     KCP 参数档（"balance"/"aggressive"，兼容别名 "balanced"）； QUIC 忽略此参数，可传 null
     */
    ...
    public static native @Nullable String remoteAddress(long conn);

    /** 取回服务端尚未上报的新连接 id 列表（每次调用后不会重复返回）。 */
    public static native long @Nullable [] acceptConnections(long server);
     ...
     */
    public static native long connect(
            int kind,
            String host,
            int port,
            @Nullable String kcpProfile
    ...
    /**
     * 写入一段字节（批量桥）：取 {@code data[0..length)} 入队， 调用方可复用暂存区避免每消息堆分配。
     *
     * @return 实际入队字节数：0 表示队列满/连接未就绪（调用方需缓冲重试，不可丢弃）； -1 表示连接不存在、已关闭，或 length 越界（&lt;0 或 &gt;
     * data.length）
     */
    public static native int writeChunk(
            long conn,
            byte[] data,
            int length
    );
    ...
    public static native byte @Nullable [] readChunk(
            long conn,
            int maxBytes
    );

    /**
     * 读取最多 maxBytes 字节写入直接缓冲区（{@link #readChunk} 的零分配 变体）：native 从 {@code buffer} 基址绝对偏移 0
     * 开始写入，不读取也 不修改其 position/limit。调用方须保证 buffer 为直接缓冲且容量 ≥ maxBytes。
     *
     * @return 实际读取字节数：0 表示暂无数据； -1 表示连接不存在、已关闭或 buffer/maxBytes 参数非法
     */
    public static native int readChunkInto(
            long conn,
            ByteBuffer buffer,
            int maxBytes
    );
