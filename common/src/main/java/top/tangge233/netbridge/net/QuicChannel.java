...
public class QuicChannel extends AbstractChannel {
    ...
    protected void doWrite(ChannelOutboundBuffer in) throws Exception {
        ...
        while (written < maxMessages) {
            ...
            if (accepted < 0) {
                in.remove(new IOException("quic write failed (conn " + connId + ", see net-bridge-native log)"));
    ...
    private void poll() {
        ...
        if (state == QuicNative.STATE_CONNECTED) {
            ...
            failConnect(new ConnectException(
                    "quic handshake failed (conn " + connId + ", see net-bridge-native log)"));
            ...
            try {
                ...
                if (n < 0) {
                    pipeline().fireExceptionCaught(new IOException(
                            "quic read failed (conn " + connId + ", see net-bridge-native log)"));
                    break;
                }
                if (n == 0) {
                    break;
    ...
    private final class QuicUnsafe extends AbstractUnsafe {
        ...
        public void connect(SocketAddress remoteAddress, SocketAddress localAddress, ChannelPromise promise) {
            ...
            try {
                ...
                if (id < 0) {
                    promise.tryFailure(new ConnectException("quic connect failed, see net-bridge-native log"));
                    return;
                }
                connId = id;
                startPoller(promise);
