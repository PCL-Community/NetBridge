...
依据：docs/adr/0001~0007 · docs/glossary.md · docs/conventions.md。
顺序即依赖序；每里程碑含验收标准，全绿才进下一个。
...
- ~~quinn_plaintext 内部 idle timeout 默认值未审计~~ 已审计（ADR-0008）：黑洞下 quinn 永不自行
  失败、KCP 无握手概念——watchdog + KCP 首字节判定为定案，无残留风险。
