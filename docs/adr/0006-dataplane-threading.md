...
状态：已接受 · 日期：2026-08-25 · 取代：pre-refactor QuicChannel 固定 5ms 轮询注释中的 ADR-0001 表述
...
- **已连接期**：活跃（本轮有数据或写队列非空）保持 5ms；连续空转按 5→10→20→40ms 步进退避，
  上限 40ms（一个 MC tick）；任何数据到达或写排队立即复位 5ms。
...
- 若 profile 实测 5ms 成为瓶颈，再评估 wakeup fd 方案（届时可考虑 Netty epoll transport 的
  native 扩展点），不在本期范围。
